...
long refresh_tach_data(uint8_t i) {
...
float FAN_PERCENT = 40;
uint8_t FAN_PWM = (uint8_t) (FAN_PERCENT *2.55);
...
void setup() {
  ...
  analogWrite(nFAN1_PWM, FAN_PWM);  // Set for low RPM
...
void loop() {
  ...
  rpm = refresh_tach_data(0);  //Prints the duration of the tac count and the count*30=RPM
  if (rpm >= 0) {
    Serial.print(tach_data_duration[0]);
    Serial.print(", ");
    Serial.println(rpm);
  }
  //  updateBlowerSpeed();
  // attempt to interfere with funciton...
