...
bool SanyoAceB97::evaluateFan(float pwm_ratio,float rpms,const float A, const float B, const float C) {
  if(pwm_ratio >=0.2){
    float expected = BLOWER_RESTRICTION * ((A*pwm_ratio*pwm_ratio) + (B*pwm_ratio) + C);
    ...
    if (DEBUG_FAN > 0) {
      ...
      CogCore::Debug<const char *>("rpm_difference (rpms - expected): ");
