...
//    console.assert(PW_W <= limitedWattage);
  ...
  bool CogTask::_run() {
    //not sure what the purpose of this code is LRK 1/31/2025
  ...
  MachineState CogTask::_updateAwaitingPower() {
    ...
    if (inBlackout) {
      ...
      // Note: ONLY if we are in the Awaiting Power mode do we check
      // the 12V-bus voltage and use it turn off the power relay completely.
      // This will TURN OFF THE DUE.
